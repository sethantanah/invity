const response = await fetch("api/record/attendance", {
  ...
  friendsList.forEach((friend, index) => {
    ...
    // WhatsApp Button
    const whatsappMessage = `From the family of the Kwabi Sarpongs, we would like to thank you all for your unwavering support and prayers to make our day a success. May Jehovah God continually bless you all. Num. 6:24-26`;
    const whatsappLink2 = document.createElement("a");
    whatsappLink2.href = `https://wa.me/${friend.phone.replace(
      /\D/g,
      ""
    )}?text=${encodeURIComponent(whatsappMessage)}`; // WhatsApp share link
    whatsappLink2.target = "_blank";
    whatsappLink2.className =
    ...
    // SMS Button
    const smsMessage = `From the family of the Kwabi Sarpongs, we would like to thank you all for your unwavering support and prayers to make our day a success. May Jehovah God continually bless you all. Num. 6:24-26`;
    const smsLink = document.createElement("a");
    smsLink.href = `sms:${friend.phone}?body=${encodeURIComponent(smsMessage)}`; // SMS share link
    smsLink.className =
